核心结论：已删除交易数据中的金额字段，仅保留数量（10000-20000000枚），优化买卖类型分布和随机逻辑，交易数据更贴合真实加密货币小额交易场景。
...
const LiveMarket = () => {
  ...
  const virtualTrades = [
    { 
      type: Math.random() > 0.45 ? 'buy' : 'sell', // 买卖比例接近真实市场（略偏向买入）
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className={`flex items-center gap-2 text-xl font-bold ${isPositive ? 'text-green-400' : 'text-red-400'}`}>
                {isPositive ? <TrendingUp className="w-5 h-5" /> : <Trending
